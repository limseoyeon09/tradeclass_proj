"""
TradeTable v8 — KSA 시간표 트레이드 시스템
변경사항:
 1) 이름+학번 대조 후 4자리 PIN 설정 → bcrypt 해싱 저장
 2) 교실 위치: data.json 의 room 필드 직접 사용 (classrooms.json 역조회 보조)
 3) 트레이드 신청 후 미리보기용 API
 4) 학번으로 시간표 검색 안내
"""
import json, os, re, time, hashlib, hmac
...
# Flask에 template/static 폴더를 _HERE 기준 절대경로로 명시
# (Render에서 작업 디렉토리가 달라도 정상 동작)
app = Flask(
    __name__,
...
# Render 등 환경에 따라 app.py 위치가 src/ 하위일 수 있으므로
# __file__ 기준 → 부모 디렉토리 → 작업 디렉토리 순으로 static/ 탐색
def _find_static_dir():
    candidates = [
        os.path.dirname(os.path.abspath(__file__)),          # app.py 옆
        os.path.dirname(os.path.dirname(os.path.abspath(__file__))),  # 한 단계 위
        os.getcwd(),                                          # 현재 작업 디렉토리
        os.path.join(os.getcwd(), "src"),                    # cwd/src
    ]
    for c in candidates:
        if os.path.isfile(os.path.join(c, "static", "data.json")):
            return c
    # 못 찾으면 __file__ 기준 그대로 (오류 메시지가 명확하게 나오도록)
    return os.path.dirname(os.path.abspath(__file__))

_HERE = _find_static_dir()
...
# ── PIN 저장소 (학번 → 해시) ───────────────────────────────────
# 실제 서비스에서는 DB에 저장해야 합니다. 현재는 인메모리.
_pin_store: Dict[str, str] = {}   # sid → hashed_pin
...
def _hash_pin(pin: str) -> str:
    """PIN을 bcrypt 또는 PBKDF2로 해싱"""
    if HAS_BCRYPT:
        return bcrypt.hashpw(pin.encode(), bcrypt.gensalt()).decode()
    else:
        # bcrypt 없을 때 fallback: PBKDF2-HMAC-SHA256
        salt = os.urandom(16).hex()
        dk = hashlib.pbkdf2_hmac("sha256", pin.encode(), salt.encode(), 260000)
...
# ────────────────────────────────────────────────────────────
# [기능 1] 로그인: 이름+학번 대조 → PIN 설정/확인
# ────────────────────────────────────────────────────────────
@app.route("/api/login", methods=["POST"])
def api_login():
    d = request.get_json(force=True)
    username = d.get("username", "").strip()
    name_input = d.get("name", "").strip()      # 이름 (첫 로그인 시)
    pin_input = d.get("pin", "").strip()        # 4자리 PIN
    action = d.get("action", "login")           # "check_name" | "set_pin" | "login" | "admin"
    ...
    # ── 이름 대조 단계 (action="check_name") ──
    if action == "check_name":
        stored_name = s.get("name", "")
        # 이름 비교: 공백·대소문자 무시
        def normalize_name(n):
            return re.sub(r"\s+", "", n).lower()
        if normalize_name(name_input) != normalize_name(stored_name):
            return jsonify({"ok": False, "error": "이름이 학번과 일치하지 않습니다."}), 401
        has_pin = sid in _pin_store
        return jsonify({"ok": True, "sid": sid, "name": stored_name, "has_pin": has_pin})

    # ── PIN 설정 단계 (action="set_pin") ──
    if action == "set_pin":
        if not re.match(r"^\d{4}$", pin_input):
            return jsonify({"ok": False, "error": "PIN은 숫자 4자리여야 합니다."}), 400
        _pin_store[sid] = _hash_pin(pin_input)
        return jsonify({"ok": True, "sid": sid, "name": s.get("name", sid), "admin": False})

    # ── 일반 로그인 (action="login" 또는 기본) ──
    if sid not in _pin_store:
        # PIN 미설정: 이름 확인 단계로 안내
        return jsonify({"ok": False, "need_setup": True, "sid": sid, "name": s.get("name", sid),
                        "error": "PIN이 설정되지 않았습니다. 이름을 확인하고 PIN을 설정하세요."})

    if not pin_input:
        return jsonify({"ok": False, "error": "PIN을 입력하세요."}), 400

    if not _check_pin(pin_input, _pin_store[sid]):
        return jsonify({"ok": False, "error": "PIN이 틀렸습니다."}), 401

    return jsonify({"ok": True, "sid": sid, "name": s.get("name", sid), "admin": False})


# ────────────────────────────────────────────────────────────
# [기능 2] 시간표 조회 (room 필드 포함)
# ────────────────────────────────────────────────────────────
...
# ────────────────────────────────────────────────────────────
# [기능 3] 트레이드 후 시간표 미리보기 API
# ────────────────────────────────────────────────────────────
@app.route("/api/preview-after-trade", methods=["POST"])
def api_preview_after_trade():
    """
    트레이드 신청 목록을 받아 변경 후 시간표 슬롯을 반환.
    enrolled: 현재 수강 목록
    requests: [{course_key, from_section, to_sections}] — to_sections[0]을 1지망으로 미리보기
    """
    d = request.get_json(force=True)
    enrolled = d.get("enrolled", [])
    trade_reqs = d.get("requests", [])

    # 1지망 분반으로 가정해 미리보기 시간표 생성
    wish_map = {}
    for req in trade_reqs:
        ck = req.get("course_key", "")
        to_sections = req.get("to_sections", [])
        if not to_sections:
            continue
        to_sec = to_sections[0]
        to_slots = COURSES.get(ck, {}).get("slots", {}).get(to_sec, [])
        # room은 classrooms.json에서 역조회
        room = ""
        for classroom, schedule in CLASSROOMS.items():
            for slot_key, course_sec in schedule.items():
                # course_sec 형식: "과목명(학년)_분반"
                # ck 형식: "과목명(학년)", to_sec: "분반번호"
                expected = f"{ck}_{to_sec}"
                if course_sec == expected:
                    room = classroom
                    break
            if room:
                break
        wish_map[ck] = {"to": to_sec, "to_slots": to_slots, "room": room}
    ...
    for e in enrolled:
        ...
        if ck in wish_map:
            w = wish_map[ck]
            after_enrolled.append({
                **e,
                "section": w["to"],
                "slots": w["to_slots"],
                "room": w["room"] or e.get("room", ""),
                "changed": True
            })
        else:
            after_enrolled.append({**e, "changed": False})

    return jsonify({"ok": True, "after_enrolled": after_enrolled, "wish_map": wish_map})
...
def health():
    return jsonify({"status":"ok","trade_requests":sum(len(v) for v in _trade_requests.values()),
        "period_open":is_period_open(),"professors":len(PROFESSORS),"bcrypt":HAS_BCRYPT})
